...
- [ ] Open `src/Xcaciv.Command.Core/CommandParameters.cs`
- [ ] Locate `ProcessOrderedParameters` method (line ~130)
- [ ] Add bounds check before accessing `parameterList[0]`:
  ```csharp
  ...
  ```
- [ ] Add unit test case for empty parameter list
- [ ] Run tests: `dotnet test src/Xcaciv.Command.Tests`
- [ ] Verify no regressions in existing tests

#### Task 1.2 - Fix ProcessSuffixParameters Bounds
- [ ] Locate `ProcessSuffixParameters` method (line ~145)
- [ ] Add similar bounds check:
  ```csharp
  if (parameterList.Count == 0) {
      if (!parameter.IsRequired && !string.IsNullOrEmpty(parameter.DefaultValue)) {
          parameterLookup.Add(parameter.Name, parameter.DefaultValue);
  ...
  ```
- [ ] Add unit test for empty suffix parameter handling
- [ ] Run full test suite

#### Task 1.3 - Add Edge Case Tests for Parameter Parsing
- [ ] Create `src/Xcaciv.Command.Tests/ParameterBoundsTests.cs`
- [ ] Add test cases:
  - Empty parameter list with required ordered parameter
  - Empty parameter list with optional ordered parameter
  - Empty parameter list with required suffix parameter
  - Single parameter when multiple required
  - Null parameter list (defensive)
- [ ] All new tests should PASS after fixes
- [ ] Run: `dotnet test src/Xcaciv.Command.Tests/ParameterBoundsTests.cs`

**Acceptance Criteria:**
- ? No `IndexOutOfRangeException` on empty parameter lists
- ? New parameter bounds tests all pass
...
- [ ] Locate `ExecuteCommand` method (line ~230)
- [ ] Move help check BEFORE try-catch block:
  ```csharp
  protected async Task ExecuteCommand(string commandKey, IIoContext ioContext, IEnvironmentContext env)
  {
      // Check for help request first (before attempting execution)
      if (ioContext.Parameters?.Length > 0 && 
          ioContext.Parameters[0].Equals($"--{this.HelpCommand}", StringComparison.CurrentCultureIgnoreCase))
      {
          if (Commands.TryGetValue(commandKey, out ICommandDescription? commandDesc))
          {
              var cmdInstance = GetCommandInstance(commandDesc, ioContext);
  ...
  ```
- [ ] Remove help logic from exception handler
- [ ] Add unit test for help request without throwing exception
- [ ] Run: `dotnet test src/Xcaciv.Command.Tests/CommandControllerTests.cs`

#### Task 1.5 - Add Help Request Test
- [ ] Add test case to `CommandControllerTests`:
  - `HelpRequestShouldNotThrowException`
  - `HelpRequestShouldOutputValidHelp`
- [ ] Verify help output contains command description and usage
...
### Phase 2b: Error Scenario Documentation

#### Task 2.5 - Add XML Documentation to Exception Cases
- [ ] Update `AbstractCommand.cs` with exception documentation:
  ```csharp
  /// <remarks>
  /// This method may throw:
  /// - ArgumentException: if required parameters are missing
  /// - InvalidOperationException: if command instance cannot be created
  /// </remarks>
  ```
- [ ] Update `CommandController.ExecuteCommand` with comprehensive exception documentation
- [ ] Update `CommandParameters.cs` method documentation
- [ ] Run: `dotnet build` to verify no doc warnings

**Acceptance Criteria:**
- ? Exception types documented for public methods
- ? Build succeeds with no warnings
- ? IntelliSense shows exception info
...
**Target Completion:** January 2024 (estimated)  
**Primary Contact:** GitHub Copilot  
**Review Recommended:** After Phase 6 (before Phase 7)
